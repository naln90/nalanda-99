...
全部端点受 :func:`require_admin_key` 保护：配置 ``ADMIN_API_KEY`` 后强制校验
``X-Admin-Key`` 头；未配置时 fail-open 放行（演示态）。
"""
...
from ..models import FraudCase, GrowthRule
from ..rules import COMPLIANCE_NOTICE, analyze_text
from ..schemas import CaseCreateRequest
from ..services import get_numeric_rule_keys
...
def admin_create_case(
    payload: CaseCreateRequest, db: Session = Depends(get_db), admin_auth: None = Depends(require_admin_key)
) -> dict[str, object]:
...
def admin_analyze_case(
    case_id: str, db: Session = Depends(get_db), admin_auth: None = Depends(require_admin_key)
) -> dict[str, object]:
...
def admin_save_rules(
    payload: dict[str, Any], db: Session = Depends(get_db), admin_auth: None = Depends(require_admin_key)
) -> dict[str, object]:
    numeric_keys = get_numeric_rule_keys()
    invalid: list[str] = []
