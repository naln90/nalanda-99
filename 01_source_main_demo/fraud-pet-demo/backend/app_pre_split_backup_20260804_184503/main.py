...
def create_app(database_url: str | None = None) -> FastAPI:
    engine = create_engine_for_url(database_url or os.getenv("DATABASE_URL") or default_database_url())
    ...
    # ── v1 测评 API（Phase 1.2：基于 AssessmentSession 的会话式测评）──

    @app.post("/api/v1/assessment/sessions")
    ...
    @app.post("/api/v1/assessment/sessions/answer")
    ...
    @app.post("/api/v1/assessment/sessions/complete")
    ...
    # ── v1 能力画像 API（Phase 1.3：五维能力画像 + 历史快照趋势）──
    @app.get("/api/v1/assessment/ability-profile")
    ...
    @app.post("/api/knowledge/analyze-image")
    ...
    @app.post("/api/admin/cases/{case_id}/analyze")
    ...
    @app.post("/api/admin/cases/{case_id}/generate-questions")
    ...
    # ---------- Ability Profile ----------
    @app.get("/api/assessment/ability-profile")
    ...
    # ---------- Task Package ----------
    @app.post("/api/task-package/generate")
    ...
    # ── v1 任务包 API（Phase 1.4：个性化任务包，含富元数据）─────────────────
    @app.post("/api/v1/task-package/generate")
    ...
    @app.get("/api/v1/task-package/current")
    ...
    @app.get("/api/task-package/current")
    ...
    @app.post("/api/task-package/items/{item_id}/complete")
    ...
    @app.post("/api/training/scenario/start")
    ...
    @app.post("/api/training/scenario/{session_id}/reply")
    ...
    @app.post("/api/training/scenario/{session_id}/finish")
    ...
    # ---------- Risk Analysis AI Enhancement ----------
    @app.post("/api/risk/analyze-ai")
    ...
    # ---------- Emergency Stop Loss ----------
    @app.get("/api/risk/emergency-stop-loss/types")
    ...
    @app.post("/api/risk/emergency-stop-loss")
    ...
    @app.get("/api/evidence/prompt-versions")
    ...
    def counselor_dashboard(db: Session = Depends(get_db)) -> dict[str, object]:
        ...
        # 按维度聚合
        dim_avg: dict[str, list[float]] = {"识诈力": [], "判断力": [], "应对力": [], "证据力": [], "求助力": []}
        ...
        for owner_id in owner_ids:
            ...
            student_profiles.append({
                ...
                "weakDimensions": json.loads(a.weak_dimensions_json),
                "accuracy": a.accuracy,
    ...
    @app.get("/api/counselor/class-meeting")
    def class_meeting_materials(db: Session = Depends(get_db)) -> dict[str, object]:
        ...
        for a in owner_latest.values():
            for dim in json.loads(a.weak_dimensions_json):
                weak_counter[dim] = weak_counter.get(dim, 0) + 1
    ...
    @app.post("/api/v1/training/scenario/start")
    ...
    @app.post("/api/v1/training/scenario/{session_id}/reply")
    ...
    @app.post("/api/v1/training/scenario/{session_id}/finish")
    ...
    @app.get("/api/v1/training/scenario/sessions/{session_id}")
