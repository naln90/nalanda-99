...
class User(Base):
    ...
    # 校园认证信息（需求#30）：学号 / 学校 / 院系，用于校园统一认证登录与用户真实性
    student_id: Mapped[str | None] = mapped_column(String, nullable=True, default=None, index=True)
    school: Mapped[str | None] = mapped_column(String, nullable=True, default=None)
    department: Mapped[str | None] = mapped_column(String, nullable=True, default=None)
...
class KnowledgeItem(Base):
    ...
    theme: Mapped[str] = mapped_column(
        String, nullable=False, index=True, default="通用"
    )
    title: Mapped[str] = mapped_column(String, nullable=False)
    risk_level: Mapped[str] = mapped_column(String, nullable=False)
...
class LearningPlan(Base):
    ...
    # 需求#11 延期申请累计天数（演示态自动通过；正式环境可改为辅导员审批）
...
class LearningPlanExtension(Base):
    """任务包延期申请记录（需求#11：进度动态—延期申请）。"""

    __tablename__ = "learning_plan_extensions"
    ...
    status: Mapped[str] = mapped_column(String, nullable=False, default="approved")  # pending | approved
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
class LearningPlanItem(Base):
    ...
    completed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    # 微课视频：命中「微课/课程/导学」类任务的真实视频（CC0 来源，可溯源）
    video_url: Mapped[str | None] = mapped_column(String, nullable=True, default=None)
    video_thumbnail: Mapped[str | None] = mapped_column(String, nullable=True,  default=None)
...
#
# 以下模型用于补齐赛道要求的功能缺口：
# 集市点赞/收藏/评分/评论(#26)、好友关系与隐私三级(#30)、
# 消息通知(#29)、项目式协作管控(#31)。


class MarketLike(Base):
    """集市条目点赞记录（用于按用户去重计数）。"""

    __tablename__ = "market_likes"
...
class TeamMember(Base):
    ...
    owner_id: Mapped[str] = mapped_column(String, nullable=False, index=True)
    role: Mapped[str] = mapped_column(String, nullable=False, default="成员")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
# ==================== 微课视频库 (赛道：学习集市·真实视频资源) ====================
#
# 以 CC0 / 免费可商用视频源（Pexels、Pixabay、Mixkit、Coverr、W3C 示例等）构建
# 一个「视频数据库」。任务包中「微课/课程/导学」类条目会按主题命中对应视频，
# 前端优先播放真实视频；播放失败时降级为原有的模拟进度播放器。
# 所有条目均标注来源与可溯源页面，便于后续替换为更符合主题的自有素材。


class VideoLibrary(Base):
    """微课视频资源库 — 主题维度的可播放视频（CC0 / 免费授权）。"""

    __tablename__ = "video_library"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    theme: Mapped[str] = mapped_column(String, nullable=False, index=True)
    # 主题关键词（与 THEME_TASK_PROFILES 的 match 对齐），用于按主题命中
