...
def v1_create_session(payload: CreateSessionRequest, db: Session = Depends(get_db)) -> dict[str, object]:
        ...
        sess, questions = create_session(db, payload.ownerId, payload.mode)
...
def v1_get_session(session_id: str, db: Session = Depends(get_db)) -> dict[str, object]:
        ...
        answers = (
            ...
            .filter(AssessmentAnswer.session_id == session_id)
...
def v1_submit_answer(payload: SubmitAnswerRequest, db: Session = Depends(get_db)) -> dict[str, object]:
        ...
        existing = (
            db.query(AssessmentAnswer)
            .filter(
                AssessmentAnswer.session_id == payload.sessionId,
                AssessmentAnswer.question_id == payload.questionId,
            )
            .first()
        )
        if existing:
        ...
        # 记录答案
        answer_record = record_answer(db, sess, question, payload.answer)
...
def v1_ability_profile(
        ...
        - 能力等级完全由规则计算，不依赖 AI
        """
        from ..ability_profile import ABILITY_DIMENSIONS, DIMENSION_DESCRIPTIONS, DIMENSION_SUGGESTIONS, normalize_profile_scores
        ...
        # 历史薄弱维度也可能是旧键名，做一次归一
        weak_dimensions = [normalize_dim_key(d) for d in weak_dimensions]
        ...
        if includeHistory:
            ...
            for snap in snapshots:
                snap_scores = normalize_profile_scores(json.loads(snap.scores_json))
                history.append({
                    "id": snap.id,
                    "scores": snap_scores,
