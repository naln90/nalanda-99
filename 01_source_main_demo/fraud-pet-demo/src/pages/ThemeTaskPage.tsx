...
/** 任务类型推断：综合标题、描述、验收标准，把主题条目映射到可执行交互 */
function inferTaskMode(item: ThemeItem): 'micro-course' | 'quiz' | 'risk-signal' | 'chat-sim' | 'reflection' | 'outcome' {
  const t = item.title || '';
...
/** 根据主题/任务信息生成题库，并解析题目数量、通过率要求（题库随主题切换，非反诈主题用中性自测） */
function buildQuiz(item: ThemeItem, themeTitle: string) {
  ...
  // 题库随主题切换：Python/编程主题用编程题库，其它主题用中性学习自测题库；
  // 非反诈主题绝不会出现“诈骗/刷单”等相关内容。
  const pool = getQuizPool(resolveThemeProfile(themeTitle), themeTitle);
...
/** 风险信号识别任务 */
function buildRiskSignal(themeTitle: string) {
  const base = themeTitle.replace(/主题|学习|任务/g, '') || '刷单诈骗';
  return {
...
/** AI 模拟对话任务 */
function buildChatSim(themeTitle: string) {
  const base = themeTitle.replace(/主题|学习|任务/g, '') || '兼职刷单';
  return {
    messages: [
...
export default function ThemeTaskPage() {
  ...
  const completeThemeItemV3 = useAppStore((s) => s.completeThemeItemV3);

  const ownerId = currentUser?.ownerId ?? '';

  useEffect(() => {
    if (!ownerId) return;
    if (!activeTheme) loadActiveTheme(ownerId);
  }, [ownerId, activeTheme, loadActiveTheme]);

  const item = useMemo(() => {
    return activeTheme?.items.find((i) => i.id === itemId) ?? null;
  }, [activeTheme, itemId]);

  const themeTitle = activeTheme?.theme?.title ?? '本月主题';
  ...
  // ===== 答题状态 =====
  const quiz = useMemo(() => (item ? buildQuiz(item, themeTitle) : null), [item, themeTitle]);
  const [answers, setAnswers] = useState<Record<string, number | null>>({});
  const [quizSubmitted, setQuizSubmitted] = useState(false);

  // ===== 风险信号状态 =====
  const risk = useMemo(() => (item ? buildRiskSignal(themeTitle) : null), [themeTitle]);
  ...
  // ===== 模拟对话状态 =====
  const chat = useMemo(() => (item ? buildChatSim(themeTitle) : null), [themeTitle]);
  ...
  const handleComplete = async () => {
    ...
    await completeThemeItemV3(item.id, ownerId);
    if (!useAppStore.getState().error) {
      success(`任务完成，+${item.energyReward} 盾能`);
    }
  };
  ...
  if (!item) {
    return (
      <div className="py-20 text-center text-subtext">
        <p>任务不存在或尚未加入主题</p>
        <Button variant="ghost" className="mt-4" onClick={() => navigate('/learning')}>
          <ArrowLeft size={16} /> 返回主题学习
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate('/learning')} className="gap-1 px-2">
          <ArrowLeft size={16} /> 返回
        </Button>
        <div className="flex-1 min-w-0">
          <h1 className="text-base font-extrabold text-ink truncate">{item.title}</h1>
          <p className="text-[11px] text-subtext truncate">{themeTitle}</p>
        </div>
        ...
        <CardContent className="space-y-2">
          <div className="flex items-center justify-between text-[11px] text-subtext">
            ...
            <Badge variant="default" className="gap-0.5">
              <Zap size={10} />+{item.energyReward} 盾能
            </Badge>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-5">
            ...
            {attemptsExhausted && !quizSubmitted && (
              <div className="rounded-lg border border-danger-200 bg-danger-50 p-3 text-center">
                ...
                <p className="text-xs text-danger-600 mt-1">请明日再来挑战，继续巩固反诈知识</p>
              </div>
            )}
            {!quizSubmitted ? (
              <Button
        ...
        {isDone ? (
          <div className="space-y-2">
            <div className="flex items-center justify-center gap-2 text-sm text-safe-600 font-semibold">
              <CheckCircle2 size={16} /> 已完成，+{item.energyReward} 盾能
            </div>
            <Button variant="outline" size="sm" onClick={() => navigate('/learning')} fullWidth>
              返回主题学习
            </Button>
          </div>
        ) : canComplete ? (
